...
If we would see a significant effect here, that would indicate the trends were not parallel before the treatment, which makes it highly likely that the parallel trends assumption is violated. 

## Part 2: Instrumental Variables (IV)

Let's clean up our environment so we don't clutter the environment.
...
data <- data %>%
    mutate(
        Z = rbinom(N, 1, 0.5), # random assignment of encouragement
        D = D0 * (1 - Z) + D1 * Z, # reveal D based on Z
        Y = Y0D0 * (1 - Z) + Y1D1 * Z # reveal Y based on Z
    )
```
...
```{r wald_estimator}
# First, let's estimate the complier proportion, we call it ITT_D because it is the ITT for the effects of Z on D
ITT_D <- mean(data$D[data$Z == 1]) - mean(data$D[data$Z == 0])

# it's just:
lm(D ~ Z, data)

# Next, estimate the ITT for the effect of Z on Y
ITT <- lm(Y ~ Z, data)$coefficients[["Z"]]

# Use the Wals estimator to get the LATE
ITT / ITT_D
```
...
# We can also do this with estimatr::iv_robust()
iv_model <- estimatr::iv_robust(Y ~ D | Z, data, diagnostic = TRUE, se = "HC2")

# Let's check the results:
iv_model %>%
    broom::tidy()

# broom::glance() can be used to retrieve some diagnostics (if we set diagnostic = TRUE in iv_robust())
# See `?glance.iv_robust()` for more details
...
We have our potential outcomes for the outcome variable, so we can assign $D$, and realize $Y$. Recall, our cutpoint is $c = 5$.
...
```{r rdplot}
rdplot(x = data$X, y = data$Y, c = 5, x.label = "Running Variable (X)", y.label = "Density")
```
...
```{r prep_ck}
ck_did <- readr::read_csv("/Users/anton/Library/CloudStorage/OneDrive-LondonSchoolofEconomics/Teaching/2025/ME314_prep/labs/seminar6/data/ck_did.csv")
...
modelsummary(list(ck_2x2, ck_twfe),
    statistic = "std.error",
    stars = T
)
```
...
ggdag::ggdag(
    dagify(
        ...
        price ~ tax + U,
        coords = list(
            x = c(smoke = 4, price = 1, tax = 1, U = 2.5),
            y = c(smoke = 0, price = 0, tax = 3, U = 3)
        )
    )
) +
    theme_void() +
...
# look at f-statistic for fist stage for relevance
broom::glance(iv_result)$statistic.weakinst
broom::glance(iv_result)$p.value.weakinst

# or simply
summary(lm(price ~ taxs, data = CigarettesSW))$fstatistic["value"]

# Now, let's create a regression table with the results:
modelsummary(iv_result, statistic = "std.error", stars = TRUE)
```
...
```{r rdplot_senate}
rdplot(y = vote_dat$vote, x = vote_dat$margin, x.label = "Margin of Victory", y.label = "Vote Share")
```
...
summary(lm_robust(vote ~ D + D * margin + D * (I(margin^2)) + D * (I(margin^3)) + D * (I(margin^4)), vote_dat))
```
